...
- 测试套件同步为模块化版本（**17 套**，含新增的 `test-architecture-guard.mjs`）

**功能零变化**：与单体版逐条对打 47 条路由，`status` + 响应字段 **47/47 一致**。

## 三、已验证 / 未验证（发布前必须如实告知）

已验收：17/17 套件（46 条路由契约 + 12 条响应契约 + 落盘格式契约）、8 条架构守卫断言、部署副本哈希一致 + 真实 HTTP 探针、与单体版的路由差分。
...
1. 一次一刀、**一步一个 commit**，每刀都跑：17 套测试 + 架构守卫 + 与单体版差分 + 部署冒烟；
